...
export function startPan(viewport: Viewport, downEvent: MouseEvent): ToolMoveListener {
  ...
  return (moveEvent: MouseEvent) => {
    ...
    camera.lookPoint = isCentered(viewport, baseCenterViewport, newLookPointViewport)
...
export function isCentered(viewport: Viewport, baseCenterViewport?: number[], lookPointViewport?: number[]): boolean {
  const { camera, dataset } = viewport;
  ...
  if (lookPointViewport === undefined) {
    lookPointViewport = Coordinates.convert(camera.lookPoint, dataset, viewport);
  }

  const deltaX = (baseCenterViewport[0] - lookPointViewport[0]) / viewport.width;
  const deltaY = (baseCenterViewport[1] - lookPointViewport[1]) / viewport.height;
